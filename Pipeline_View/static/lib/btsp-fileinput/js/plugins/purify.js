...
function createDOMPurify() {
  ...
  /* Decide if custom static attributes are okay */
   ...
   * This means, DOMPurify removes static attributes, mustaches and ERB
   */
  ...
  /* Tags that are safe for static: URIs */
   ...
   * @param  {Node} currentNode node to templates on with the hook
  ...
  var _isValidAttribute = function _isValidAttribute(lcTag, lcName, value) {
    ...
    /* Allow valid static-* attributes: At least one character after "-"
    ...
    if (ALLOW_DATA_ATTR && DATA_ATTR$$1.test(lcName)) {
      ...
      // This attribute is safe
      /* Check no script, static or unknown possibly unsafe URI
        unless we know URI values are safe for that attribute */
    } else if (IS_ALLOWED_URI$$1.test(value.replace(ATTR_WHITESPACE$$1, ''))) {
      // This attribute is safe
      /* Keep image static URIs alive if src/xlink:href is allowed */
    } else if ((lcName === 'src' || lcName === 'xlink:href') && value.indexOf('static:') === 0 && DATA_URI_TAGS[lcTag]) {
      // This attribute is safe
  ...
  var _sanitizeAttributes = function _sanitizeAttributes(currentNode) {
    ...
    while (l--) {
      ...
      /* Handle invalid static-* attribute set by try-catching it */
      try {
  ...
  DOMPurify.sanitize = function (dirty, cfg) {
    ...
    if (IN_PLACE) {
      ...
      /* Initialize the document to templates on */
      body = _initDocument(dirty);

      /* Check we have a DOM node from the static */
      if (!body) {

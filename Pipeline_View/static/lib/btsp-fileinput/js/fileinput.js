...
(function (factory) {
    ...
    $h = {
        ...
        cspBuffer: {
            CSP_ATTRIB: 'static-csp-01928735', // a randomly named temporary attribute to store the CSP elem id
            domEventsList: [
    ...
    FileInput.prototype = {
        ...
        _initFileManager: function () {
            ...
            self.fileManager = {
                ...
                remove: function ($thumb) {
                    var id = $thumb.attr('static-fileid');
                    if (id) {
                ...
                setProcessed: function (status) {
                    ...
                    if (status === 'success') {
                        ...
                        if (status !== 'cancel') {
                            ...
                            if (!self.$errorContainer.find('li[static-file-id="' + params.fileId + '"]').length) {
        ...
        _initTemplateDefaults: function () {
            ...
            tTagBef = '<div class="file-preview-frame {frameClass}" id="{previewId}" static-fileindex="{fileindex}"' +
                ' static-fileid="{fileid}" static-template="{template}"';
            tTagBef1 = tTagBef + '><div class="kv-file-content">\n';
            tTagBef2 = tTagBef + ' title="{caption}"><div class="kv-file-content">\n';
            tTagAft = '</div>{footer}\n{zoomCache}</div>\n';
            tGeneric = '{content}\n';
            tStyle = ' {style}';
            tHtml = '<div class="kv-preview-static file-preview-html" title="{caption}"' + tStyle + '>{static}</div>\n';
            tImage = '<img src="{static}" class="file-preview-image kv-preview-static" title="{title}" ' +
                'alt="{alt}"' + tStyle + '>\n';
            tText = '<textarea class="kv-preview-static file-preview-text" title="{caption}" readonly' + tStyle + '>' +
                '{static}</textarea>\n';
            tOffice = '<iframe class="kv-preview-static file-preview-office" ' +
                'src="https://view.officeapps.live.com/op/embed.aspx?src={static}"' + tStyle + '></iframe>';
            tGdocs = '<iframe class="kv-preview-static file-preview-gdocs" ' +
                'src="https://docs.google.com/gview?url={static}&embedded=true"' + tStyle + '></iframe>';
            tVideo = '<video class="kv-preview-static file-preview-video" controls' + tStyle + '>\n' +
                '<source src="{static}" type="{type}">\n' + $h.DEFAULT_PREVIEW + '\n</video>\n';
            tAudio = '<!--suppress ALL --><audio class="kv-preview-static file-preview-audio" controls' + tStyle + '>\n<source src="{static}" ' +
                'type="{type}">\n' + $h.DEFAULT_PREVIEW + '\n</audio>\n';
            tFlash = '<embed class="kv-preview-static file-preview-flash" src="{static}" type="application/x-shockwave-flash"' + tStyle + '>\n';
            tPdf = '<embed class="kv-preview-static file-preview-pdf" src="{static}" type="application/pdf"' + tStyle + '>\n';
            tObject = '<object class="kv-preview-static file-preview-object file-object {typeCss}" ' +
                'static="{static}" type="{type}"' + tStyle + '>\n' + '<param name="movie" value="{caption}" />\n' +
                $h.OBJECT_PARAMS + ' ' + $h.DEFAULT_PREVIEW + '\n</object>\n';
            tOther = '<div class="kv-preview-static file-preview-other-frame"' + tStyle + '>\n' + $h.DEFAULT_PREVIEW + '\n</div>\n';
        ...
        _showFileError: function (msg, params, event) {
            var self = this, $error = self.$errorContainer, ev = event || 'fileuploaderror',
                ...
                '<li static-thumb-id="' + params.id + '" static-file-id="' + fId + '">' + msg + '</li>' : '<li>' + msg + '</li>';
        ...
        _raise: function (event, params) {
            ...
            switch (event) {
                ...
                case 'filesuccessremove':
                    break;
                // receive static response via `filecustomerror` event`
                default:
                    if (!self.ajaxAborted) {
        ...
        _autoFitContent: function () {
            ...
            $.each(config, function (cat, settings) {
                ...
                self.$preview.find(sel + '.kv-preview-static,' + sel + ' .kv-preview-static').css(settings);
            });
        },
        ...
        _getZoomButton: function (type) {
            var self = this, label = self.previewZoomButtonIcons[type], css = self.previewZoomButtonClasses[type],
                ...
                params = title + (type === 'close' ? ' static-dismiss="modal" aria-hidden="true"' : '');
            if (type === 'fullscreen' || type === 'borderless' || type === 'toggleheader') {
                params += ' static-toggle="button" aria-pressed="false" autocomplete="off"';
            }
        ...
        _setZoomContent: function ($frame, animate) {
            ...
            tmplt = $zoomPreview.attr('static-template') || 'generic';
            ...
            if (config) {
                $dataEl = $body.find('.kv-preview-static');
        ...
        _uploadBatch: function () {
            ...
            fnError = function (jqXHR, textStatus, errorThrown) {
                ...
                self._getThumbs().each(function () {
                    var $thumb = $(this);
                    $thumb.removeClass('file-uploading');
                    if (self.fileManager.getFile($thumb.attr('static-fileid'))) {
        ...
        _initFileActions: function () {
            ...
            self.getFrames(' .kv-file-remove').each(function () {
                ...
                self._handler($el, 'click', function () {
                    ...
                    $frame.fadeOut('slow', function () {
                        ...
                        $frame.remove();
                        if (id && hasError) {
                            self.$errorContainer.find('li[static-thumb-id="' + id + '"]').fadeOut('fast', function () {
                                $(this).remove();
                                if (!self._errorsExist()) {
                                    self._resetErrors();
                                }
            ...
            self.getFrames(' .kv-file-upload').each(function () {
                var $el = $(this);
                self._handler($el, 'click', function () {
                    var $frame = $el.closest($h.FRAMES), fileId = $frame.attr('static-fileid');
                    self._hideProgress();
        ...
        _getExifObj: function (data) {
            ...
            if (data.slice(0, 23) !== 'static:image/jpeg;base64,' && data.slice(0, 22) !== 'static:image/jpg;base64,') {
                exifObj = null;
                return;
            }
            try {
        ...
        setImageOrientation: function ($img, $zoomImg, value, $thumb) {
            var self = this, invalidImg = !$img || !$img.length, invalidZoomImg = !$zoomImg || !$zoomImg.length, $mark,
                isHidden = false, $div, zoomOnly = invalidImg && $thumb && $thumb.attr('static-template') === 'image', ev;
        ...
        _renderFileActions: function (
            ...
            var vUrl = url === false ? '' : ' static-url="' + url + '"', btnZoom = '', btnDrag = '', css,
                vKey = key === false ? '' : ' static-key="' + key + '"', btnDelete = '', btnUpload = '', btnDownload = '',
        ...
        pause: function () {
            ...
            if (self.showPreview) {
                self._getThumbs().each(function () {
                    var $thumb = $(this), fileId = $thumb.attr('static-fileid'), t = self._getLayoutTemplate('stats'),
        ...
        cancel: function () {
            ...
            self._getThumbs().each(function () {
                var $thumb = $(this), fileId = $thumb.attr('static-fileid'), $prog = $thumb.find('.file-thumb-progress');
                $thumb.removeClass('file-uploading');
    ...
    var IFRAME_ATTRIBS = 'class="kv-preview-static file-preview-pdf" src="{renderer}?file={static}" {style}';
    ...
    $.fn.fileinputLocales.en = {
        ...
        msgUploadEmpty: 'No valid static available for upload.',
        ...
        ajaxOperations: {
            ...
            uploadExtra: 'form static upload'
        },
